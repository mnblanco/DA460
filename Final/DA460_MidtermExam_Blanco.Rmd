...
The proportion for Divorced, Single, Married, and Widowed are `r round(ans["Divorced"]*100, 3)`%, `r round(ans["Single"]*100, 3)`%, `r round(ans["Married"]*100, 3)`% and `r round(ans["Widowed"]*100, 3)`% respectively.  There are roughly twice as many married than single. There are roughly twice as many single than windowed. There are roughly five times as many married than divorced.
...
The number of simulations did affect the proportions.  The proportions has less variability with large sample sizes than with small sample sizes.  The proportion from the larger sample will be closer to the corresponding probablity.
...
Suppose the variable pop2010 has a normal distribution, what is the probability that pop2010 is greater than 102,410? What is the probability that pop2010 is between 190,000 and 1,000,000?
